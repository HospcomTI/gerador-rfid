...
export async function imprimirEnderecosApi(): Promise<void> {
  const tipo = TipoComissionador.ENDERECO;
  ...
  selecionadas
    .reduce((acc, linha) => {
        const binCode = String(linha[1] || '').trim();
        if (!binCode || acc.has(binCode)) return acc;
